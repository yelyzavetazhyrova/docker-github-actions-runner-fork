...
# --- ШАГ 1: Определяем функцию очистки БЕЗ команды 'exit' ---
cleanup() {
  echo "Script is exiting. Deregistering runner..."
...
# --- ШАГ 2: Устанавливаем ловушку на EXIT. Все старые trap-функции УДАЛЕНЫ ---
if [[ "${_DISABLE_AUTOMATIC_DEREGISTRATION}" == "false" ]]; then
  if [[ "${_DEBUG_ONLY}" == "false" ]]; then
    trap 'cleanup' EXIT
  fi
...
# --- ШАГ 3: Запускаем раннер в фоне и ждем его завершения ---
start_runner() {
...
# Запускаем функцию в фоновом режиме (&) и ждем ее завершения (wait $!)
start_runner "$@" &
wait $!